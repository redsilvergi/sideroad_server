...
const getLdc = async (ldc) => {
  ...
  const qry = `SELECT * FROM public.ldctable WHERE uid = ${ldcint} and inuse = true`;
  const { rows } = await client.query(qry);
...
const getTouchedLinks = async (nf_id) => {
  try {
    ...
    const qry = `WITH RECURSIVE connected_lines AS (
                  SELECT nf_id, road_nm, geom
                  FROM public.side1r
                  WHERE nf_id = $1

                  UNION

                  SELECT s.nf_id, s.road_nm, s.geom
                  FROM public.side1r s
                  JOIN connected_lines c ON ST_Touches(s.geom, c.geom)
                  WHERE s.nf_id != c.nf_id AND s.road_nm = c.road_nm AND s.is_pfr <> 1
                )
                SELECT nf_id FROM connected_lines;`;
