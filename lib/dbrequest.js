...
const getCsv = async (qry) => {
  ...
  try {
    ...
    const columns = Object.keys(result.rows[0].slice(0, 2));
  ...
  if (ldc === null) {
    ...
    switch (col) {
      ...
      FROM (
          SELECT 
              COUNT(*)::INT AS total_count,
              COUNT(*) FILTER (WHERE sdwk_se = 1) AS x1,
              COUNT(*) FILTER (WHERE sdwk_se = 3) AS x2,
              COUNT(*) FILTER (WHERE sdwk_se = 2) AS x3
          FROM srvy.side1r
          WHERE sido = ${ldc.slice(0, 2)}
      ) ag
      CROSS JOIN LATERAL UNNEST(
...
const postSrvy = async (body) => {
  ...
  var qry = `INSERT INTO srvy.srvy (
        nfidlst, timestamp, roadLength, roadWidth, pedSeparation, slope, accTotal, accDeath, accSerious, accMinor, accReport, trafficVolume, parking, pedestrianVolume, cctv, cctvCount, securityLight, securityLightCountGood, securityLightCountBad, packageState, speedReductionFacil, pedSign, light, bench, plant, obstacle, illegalParking
  ...
  try {
    ...
    // console.log('dbreq result.rows[0]', result.rows[0]);
