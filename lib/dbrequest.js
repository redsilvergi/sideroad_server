...
// HELPERS -----------------------------------------------------
// REQUEST METHODS -----------------------------------------------------------
const getMbkey = async () => {
  try {
    // console.log('getMbkey:', config_obj.MAPBOX_SECRET);
    return config_obj.MAPBOX_SECRET;
  } catch (e) {
    console.error('getMbkey error at dbrequest: ', e);
  }
};
...
const getLength2 = async (body) => {
  ...
  // querying all or region all
  let count0 = 0;
  let count1 = 0;

  for (let i = 0; i < 5; i++) {
    if (rnfo0 && rnfo0[i]) count0++;
    if (rnfo1 && rnfo1[i]) count1++;
  }

  if ((!rnfo0 && count1 === 5) || (!rnfo1 && count0 === 5)) {
    if (!ldc) {
      return 59684670;
    } else {
      try {
...
const getCsvPfr = async (sggid) => {
  try {
    ...
    WITH ranked_results AS (
      ...
      DENSE_RANK() OVER (ORDER BY ped_fitr DESC) AS lst_rank
      FROM public.side1r
      WHERE leglcd_se = $1
      AND road_lt >= 50
      AND road_se <> 11
    ...
    SELECT *
    FROM filtered_rows
    ORDER BY ped_fitr DESC, ped_fit DESC;`;
...
const getCordOnly = async (params) => {
  const qry = `SELECT long, lat FROM public.side10 where NF_ID = $1;`;
  try {
    const result = await client.query(qry, [params]);
    const newRow = result.rows[0];
    return newRow;
  } catch (err) {
    console.log(err);
  }
  // console.log('getCordOnly params at dbrequest: ', params);
  // const qry = `SELECT long, lat FROM public.side10 where NF_ID in ('${params}');`;
  // console.log('getCordOnly qry at dbreques: ', qry);
  // const { rows } = await client.query(qry);
  // console.log('getCordOnly dbreq \n', rows[0]);
  // return rows[0];
};
...
const getSrchId = async (params) => {
  // console.log('getSrchId params at dbrequest: ', params);
  const qry = `select nf_id from public.side1r where road_nm = '${params}'`;
  const { rows } = await client.query(qry);
    ...
    SELECT 
      jsonb_build_object(
        ...
        'features', jsonb_agg(
          jsonb_build_object(
            'type', 'Feature',
            'geometry', ST_AsGeoJSON(ST_Simplify(geom, 0.002))::jsonb,
            'properties', jsonb_build_object(
              'sig_cd', sig_cd,
...
const getLdc = async (ldc) => {
  ...
  const qry = `SELECT * FROM public.ldctable WHERE ldc = ${ldcint} and inuse = true`;
  const { rows } = await client.query(qry);
...
const getRiskPrcnt = async (body) => {
  const { col, ldc } = body;

  const sido = ldc && Number(ldc.slice(0, 2));

  const qry0 = `select * from risk.${col}_prcnt;`;

  const qry1 = `select ${col} as val, count(*) as cnt,round((count(*)::numeric / sum(count(*)) over())*100,1) as prcnt from risk.side1r_length where sido = ${sido}  group by ${col};`;

  const qry2 = `select ${col} as val, count(*) as cnt,round((count(*)::numeric / sum(count(*)) over())*100,1) as prcnt from risk.side1r_length where leglcd_se = '${ldc}' group by ${col};`;

  const finalQry = ldc === null ? qry0 : ldc.slice(2) === '000' ? qry1 : qry2;
  try {
    // console.log('getRiskPrcnt triggered');
    // console.log('getRiskPrcnt finalQry\n', finalQry);

    const result = await client.query(finalQry);
    // console.log('dbreq result', result);

    const newRow = result.rows;

    // console.log('dpreq getRiskPrcnt newRow/n', newRow);
    ...
    SELECT 
      jsonb_build_object(
        ...
        'features', jsonb_agg(
          jsonb_build_object(
            'type', 'Feature',
            'geometry', ST_AsGeoJSON(p.geom, 6)::jsonb,
            'properties', jsonb_build_object(
              'id', p.uid,
              'park_se', p.park_se
            )
...
const getTopPfr = async (sggid) => {
  try {
    ...
    WITH ranked_results AS (
      ...
      ped_fitr,
      ped_fitr_rank,
      ROW_NUMBER() OVER (PARTITION BY ped_fitr_rank ORDER BY ped_fitr DESC, ped_fit DESC) AS row_num
      FROM public.side1r
      WHERE leglcd_se = $1
      AND road_lt >= 50
      AND road_se <> 11
    ...
    SELECT nf_id, ped_fit, ped_fitr, ped_fitr_rank, road_nm
    FROM filtered_rows
    ORDER BY ped_fitr DESC, ped_fit DESC;`;
    ...
    const qry = `
      ...
      FROM (
        SELECT ST_Difference(
                ST_Buffer(
                  ST_Union(ST_Buffer(geom::geography, 3000)::geometry),
                  0
                ),
                ST_Buffer(
...
const getSrvData = async (ids) => {
  try {
    ...
    WITH buffer_polygon AS (
      SELECT ST_Union(ST_Buffer(geom::geography, 100)::geometry) AS geom
      FROM public.side1r
      WHERE nf_id IN (${placeholders})
    )
    SELECT jsonb_build_object(
      'bld', (
        ...
        WHERE b.geom && buffer_polygon.geom
        AND ST_Within(b.geom, buffer_polygon.geom)
      ),
      'rodway', (
        SELECT jsonb_build_object(
          'type', 'FeatureCollection',
        ...
        WHERE r.geom && buffer_polygon.geom
        AND ST_Within(r.geom, buffer_polygon.geom)
      ),
      'pedpath', (
        SELECT jsonb_build_object(
          'type', 'FeatureCollection',
        ...
        WHERE p.geom && buffer_polygon.geom
        AND ST_Within(p.geom, buffer_polygon.geom)
      ),
      'cctv', (
        SELECT jsonb_build_object(
          'type', 'FeatureCollection',
        ...
        WHERE c.geom && buffer_polygon.geom
        AND ST_Within(c.geom, buffer_polygon.geom)
      ),
      'crosswalk', (
        SELECT jsonb_build_object(
          'type', 'FeatureCollection',
          'features', jsonb_agg(
            jsonb_build_object(
              'type', 'Feature',
              'geometry', ST_AsGeoJSON(cr.geom, 6)::jsonb,
              'properties', jsonb_build_object(
                'id', cr.uid
              )
            )
        ...
        WHERE cr.geom && buffer_polygon.geom
        AND ST_Within(cr.geom, buffer_polygon.geom)
      )
    ) AS geojson;
    `;
...
const getPfrProps = async (ids) => {
  ...
  try {
    const qry = `
    SELECT 
      SUM(cartrk_co) AS cartrk_co_sum,
      SUM(road_bt) AS road_bt_sum,
      SUM(COALESCE(rdnet_ac, 0) * road_lt) / SUM(road_lt) AS rdnet_ac_weighted_avg,
      SUM(pbuld_fa * road_lt) / SUM(road_lt) AS pbuld_fa_weighted_avg,
      SUM(bulde_de * road_lt) / SUM(road_lt) AS bulde_de_weighted_avg,
      SUM(COALESCE(pubtr_ac, 0) * road_lt) / SUM(road_lt) AS pubtr_ac_weighted_avg,
      SUM(road_lt) AS total_road_length
    FROM 
      srvy.side1r 
    WHERE nf_id = ANY ($1);
  `;
    ...
    const avrg = {
      cartrk_co: Number((row.cartrk_co_sum / ids.length).toFixed(4)), // 차로 수
      road_bt: Number((row.road_bt_sum / ids.length).toFixed(4)), // 도로 폭
      rdnet_ac: Number(row.rdnet_ac_weighted_avg.toFixed(4)), // 네트워크접근성
      pbuld_fa: Number(row.pbuld_fa_weighted_avg.toFixed(4)), // 보행유인연면적
      bulde_de: Number(row.bulde_de_weighted_avg.toFixed(4)), // 건물출입구밀도
      pubtr_ac: Number(row.pubtr_ac_weighted_avg.toFixed(4)), // 대중교통접근성
    };

    return avrg;
  } catch (err) {
    console.error('Error in getPfrProps at dbrequest: ', err);
    throw err;
  }
};

const getTouchedLinks = async (nf_id) => {
  try {
    if (!nf_id || nf_id.length === 0) {
      return [];
    }

    const qry = `WITH RECURSIVE connected_lines AS (
      SELECT nf_id, road_nm, geom, is_pfr
      FROM public.side1r
      WHERE nf_id = $1

      UNION

      SELECT s.nf_id, s.road_nm, s.geom, s.is_pfr
      FROM public.side1r s
      JOIN connected_lines c ON ST_Touches(s.geom, c.geom)
      WHERE s.nf_id != c.nf_id AND s.road_nm = c.road_nm
    )
    SELECT nf_id
    FROM connected_lines
    WHERE is_pfr <> 1;`;

    const result = await client.query(qry, [nf_id]);
    return result.rows;
  } catch (e) {
    console.error('Error in getTouchedLinks at dbrequest: ', e);
    throw e;
  }
};

// const getNfidLstByRoadnm = async (road_nm) => {
//   try {
//     if (!road_nm) {
//       return [];
//     }

//     const qry = `SELECT nf_id FROM public.side1r WHERE road_nm = $1;`;

//     const result = await client.query(qry, [road_nm]);
//     return result.rows;
//   } catch (e) {
//     console.error('Error in getNfidLstByRoadnm at dbrequest: ', e);
//     throw e;
//   }
// };
...
module.exports = {
  ...
  getTouchedLinks: getTouchedLinks,
};
